...
export function SocialLoginButtons() {
  ...
  const handleSocialLogin = async (provider: 'google' | 'azure' | 'apple' | 'slack') => {
    ...
    try {
      // Use explicit origin for redirect to avoid mismatches
      const redirectUrl = `${window.location.origin}/`;

      console.log('🔐 [SocialLogin] Initiating OAuth flow', {
        provider,
        redirectUrl,
      ...
      if (error) {
        console.error('❌ [SocialLogin] Error initiating login:', error);
        ...
        console.log('✅ [SocialLogin] OAuth flow initiated successfully', data);
      }
    } catch (error: any) {
      console.error('❌ [SocialLogin] Unexpected error:', error);
      toast.error('An unexpected error occurred. Please try again.');
    } finally {
      // Note: We don't unset loading here immediately if successful 
      // because the page will redirect. 
      // But if it fails fast or we want to be safe:
