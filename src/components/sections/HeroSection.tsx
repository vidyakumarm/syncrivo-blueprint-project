...
export function HeroSection() {
  ...
  useEffect(() => {
    const animatePackets = () => {
      setPackets((prev) => prev.map((p) => ({ ...p, progress: p.progress + 1.5 })).filter((p) => p.progress <= 100));
    };
      ...
      <div className="relative z-10 max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 w-full">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 shadow-sm">
            ...
            <span className="text-sm font-medium text-slate-600 dark:text-slate-400">Enterprise-Grade Security</span>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <div
            className={`space-y-6 text-center lg:text-center transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
          >
            ...
            <div className="space-y-5">
              <h1 className="text-4xl sm:text-5xl lg:text-[3.25rem] xl:text-[3.5rem] font-bold leading-[1.1] tracking-tight text-foreground">
                Unified Messaging Hub for and more
              ...
              <p className="text-lg sm:text-xl text-muted-foreground max-w-xl mx-auto leading-relaxed font-light">
                Sync conversations securely across platforms — reliable, compliant, and effortless. Trusted by
          ...
          >
            ...
            <div className="relative w-[380px] h-[380px] sm:w-[420px] sm:h-[420px]">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div
                  className={`absolute w-64 h-64 rounded-full border border-slate-200/50 dark:border-slate-700/30 transition-all duration-700 ${hubPulse ? "scale-110 opacity-100" : "scale-100 opacity-60"}`}
                />
                <div
                  className={`absolute w-52 h-52 rounded-full border border-slate-300/40 dark:border-slate-600/20 transition-all duration-500 ${hubPulse ? "scale-105 opacity-100" : "scale-100 opacity-50"}`}
                />
                <div
                  className={`w-44 h-44 bg-gradient-radial from-primary/15 via-primary/5 to-transparent rounded-full blur-2xl transition-all duration-700 ${hubPulse ? "scale-125 opacity-90" : "scale-100 opacity-60"}`}
                />
              </div>
            ...
            >
              ...
              <div className="flex -space-x-1.5">
                {[0, 1, 2].map((i) => (
                  <div
                    key={i}
                    className="w-5 h-5 rounded-full bg-slate-200 dark:bg-slate-700 border-2 border-slate-100 dark:border-slate-800 flex items-center justify-center"
                  >
